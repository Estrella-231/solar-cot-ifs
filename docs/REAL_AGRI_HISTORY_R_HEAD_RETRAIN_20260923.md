...
Here “clear-sky COT” means only pixels whose CPP reference COT is `<0.1` (648,648 pixels), not a clear-weather/GHI subset. On these pixels, mean reference COT was 0.00225; mean predicted COT rose from 0.1553 with frozen R to 0.1644 with continued R. Mean positive bias rose from 0.1530 to 0.1622, and the fraction predicted at COT≥0.1 rose from 11.79% to 12.98%. Thus the measured degradation is concentrated in small positive retrievals over near-zero-reference pixels. The current audit does not isolate whether those are caused by cloud-edge ambiguity, atmospheric/surface effects, or CPP-label behavior, nor whether they materially affect GHI.
