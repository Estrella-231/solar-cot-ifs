...
def independent_indices(lat, lon, target_lat, target_lon):
    ...
    # Every target pixel independently chooses its nearest coordinate. This
    # does not trust the production first-column/first-row mapping shortcut.
    rows = np.argmin(np.abs(lat[:, None, None] - target_lat[None]), axis=0)
    cols = np.argmin(np.abs(lon[:, None, None] - target_lon[None]), axis=0)
...
def check_one(sample, position, packed, arrays, nc_hash_cache):
    ...
    with netCDF4.Dataset(source_path) as ds:
        ...
        # Independent scalar indexing is unambiguous about orthogonal/fancy
        # indexing and dimension order; only 256 NC pixels are decoded.
        source_cot = np.empty((16, 16), dtype=np.float32)
        for y in range(16):
            for x in range(16):
                rc = (int(rows[y, x]), int(cols[y, x]))
                if tuple(cot_var.dimensions) == (lon_dim, lat_dim):
                    rc = rc[::-1]
                source_cot[y, x] = np.ma.filled(cot_var[rc], np.nan)
