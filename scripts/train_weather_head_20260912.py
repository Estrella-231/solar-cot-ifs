...
def main():
    ...
    out=ROOT/'runs/weather_coverage_head_seed42_20260912_v1';out.mkdir(exist_ok=False)
    ...
    for k in ('x','g','station','lead','split'):
        ...
        arrays[k]=np.load(pack/audit['arrays'][k]['name'],mmap_mode='r')
    ...
    data={k:torch.from_numpy(np.array(arrays[k])).cuda() for k in ('x','g','station','lead')}
