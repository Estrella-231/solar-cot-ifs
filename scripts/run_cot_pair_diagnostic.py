...
def load_contracts(root, protocol_sha, report):
    ...
    recorded_pack = Path(meta['pack'])
    if not recorded_pack.is_absolute():
        recorded_pack = root / recorded_pack  # The frozen PBS command runs from project root.
    require(recorded_pack.resolve() == pack and meta['norm'] == norm, 'R pack/norm identity')
    report['checkpoint_pack_path'] = dict(recorded=meta['pack'], resolved=str(recorded_pack.resolve()),
        interpretation='relative checkpoint pack paths resolve from the frozen training project root')
