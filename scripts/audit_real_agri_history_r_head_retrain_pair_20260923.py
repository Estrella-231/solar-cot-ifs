...
def main(root, run, repetitions, seed):
    ...
    completions = {}
    for arm in ARMS:
        complete = json.loads((run / arm / "complete.json").read_text())
    ...
    for station in STATIONS:
        ...
        for group in GROUPS:
            cells = {}
            for arm in ARMS:
                path = run / arm / f"{station}_{group}_validation_predictions.npz"
    ...
    for group in GROUPS:
        ...
        for _ in range(repetitions):
            sampled = rng.choice(shared_days, size=len(shared_days), replace=True)
            draw = {}
            for arm in ARMS:
                station_rmse = []
                for station in STATIONS:
                    ...
                    day_ids, inverse = np.unique(item["days"], return_inverse=True)
                    day_sse = np.bincount(inverse, weights=err ** 2, minlength=len(day_ids))
                    day_n = np.bincount(inverse, minlength=len(day_ids))
                    lookup = {day: i for i, day in enumerate(day_ids)}
                    idx = np.array([lookup[day] for day in sampled])
                    station_rmse.append(float(np.sqrt(day_sse[idx].sum() / day_n[idx].sum())))
                draw[arm] = float(np.mean(station_rmse))
            deltas.append(draw["continued_R"] - draw["original_R"])
...
if __name__ == "__main__":
    ...
    main(args.root, args.run, args.repetitions, args.seed)
