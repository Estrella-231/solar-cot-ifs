...
import numpy as np
import torch
import train_head_ghi_multiseed_core as core
...
def objective_witness():
    ...
    for run_seed in (43,44):
        core.seed(run_seed);a=core.Head(False)
        core.seed(run_seed);b=core.Head(True)
        assert all(torch.equal(v,b.state_dict()[k]) for k,v in a.state_dict().items())
        vectors.append(torch.cat([v.flatten() for v in a.state_dict().values()]))
    ...
    return dict(state='PASS_OBJECTIVE_VALUE_GRADIENT_AND_ARCHITECTURE', scale_W_m2=1000., loss=float(loss),
                paired_initialization_seeds=[43,44],different_seed_parameters_verified=True)

def run(root, cohort, run_seed):
    ...
    out = root/'runs'/f'head_ghi_multiseed_{cohort}_seed{run_seed}_20260910_v1'
    ...
    metadata=dict(old_contract, code_sha256=core.sha(__file__), imported_head_sha256=core.sha(core.__file__),
                  original_head_sha256=EXPECTED_ORIGINAL, cohort=cohort, seed=run_seed,
                  learning_rates=[0.0003 if cohort=='original' else 0.001],
                  smoke_seed=42, seed42_source='head_ghi_'+cohort+'_seed42_20260910_v1',
                  ...
                  protocol_sha256=core.sha(root/'docs/HEAD_GHI_MULTISEED_PROTOCOL_20260910.md'))
    ...
    print('PACK_READY',cohort,'seed',run_seed,len(train),len(val),str(out),flush=True)
    ...
    for group in ('A','AC'):
        for lr in metadata['learning_rates']: core.fit(data,train,val,out,group,lr,2048,metadata)
...
def main():
    ...
    for run_seed in (43,44):
        for cohort in ('original','qc1'):run(args.root,cohort,run_seed)
    print('GHI_MULTISEED_TRAINING_COMPLETE',flush=True)
