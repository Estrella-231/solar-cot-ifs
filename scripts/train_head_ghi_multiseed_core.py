"""GHI-loss-only fork of immutable train_head_pilot.py; architecture and fit schedule unchanged."""
...
def ghi_objective(pred, truth, clear, weight):
    return (((pred.float()*clear.float()-truth.float())/1000.0).square()*weight.float()).mean()

def update(model,opt,data,ids):
    opt.zero_grad(set_to_none=True)
    p=prediction(model,data,ids)
    # FP32 residual normalized by fixed 1000 W/m2; no division by small clear-sky values.
    loss=ghi_objective(p, data['ghi'][ids], data['clear'][ids], data['weight'][ids])
    if not torch.isfinite(loss):raise RuntimeError('nonfinite weighted normalized GHI loss')
    loss.backward()
    # No added clipping or robust loss: preserve preregistered weighted normalized GHI MSE.
...
def weighted_loss(model,data,ids):
    ...
    return float(ghi_objective(p, data['ghi'][ids], data['clear'][ids], data['weight'][ids]))
...
def overfit(data,train,out):
    ...
    result=dict(passed=all(r['passed'] for r in reports),sequences=selected.cpu().tolist(),rows=len(ids),
        ...
        gate='final weighted normalized GHI MSE <= half initial for both A and AC',groups=reports)
    atomic(path,result)
...
def fit(data,train,val,out,group,lr,batch,metadata):
    ...
    run=out/f'{group}_lr{lr:g}_seed{run_seed}';run.mkdir(parents=True,exist_ok=True)
    if (run/'complete.json').exists():
        ...
        return
    seed(run_seed);model=Head(group=='AC').cuda();opt=torch.optim.AdamW(model.parameters(),lr=lr,weight_decay=1e-4)
    ...
    for epoch in range(epoch0,50):
        if wait>=8:break
        gen=torch.Generator(device='cuda').manual_seed(run_seed+epoch)
        ...
        ck=dict(model=model.state_dict(),optimizer=opt.state_dict(),metadata=metadata,group=group,lr=lr,
                best=best,wait=wait,next_epoch=epoch+1,batch=batch,seed=run_seed)
        if improved:save(run/'best.pt',ck)
        save(run/'last.pt',ck)
        report=dict(epoch=epoch,train_weighted_normalized_ghi_mse=loss_sum/len(train),**result,best=best,wait=wait,seconds=time.monotonic()-start)
