"""Paired real-AGRI retrieval pilot with extra log-loss weight on thick cloud.

Both arms start from the same frozen Hunan R checkpoint and use the same
real observed AGRI training/validation inputs, order, optimizer and selector.
The sole arm difference is a fixed CPP-label-derived pixel weight in training.
No forecast AGRI or test payload is opened.
"""
import argparse
import csv
import hashlib
...
ARMS = ("log_control", "high_cloud_weighted")
...
def train_loss(log_pred, log_true, mask, arm, high_weight):
    values = F.huber_loss(log_pred, log_true, delta=0.1, reduction="none")
    if arm == "log_control":
        return masked_mean(values, mask)
    physical_true = torch.expm1(log_true)
    weights = 1.0 + high_weight * torch.clamp(physical_true / 30.0, 0.0, 1.0)
    return (values * weights * mask).sum() / (weights * mask).sum().clamp_min(1)
...
def main():
    ...
    parser.add_argument("--high-weight", type=float, default=2.0)
    ...
    contract = {"state": "PILOT_RUNNING", "test_used": False, "seed": args.seed,
                ...
                "lr": args.lr, "high_weight": args.high_weight,
                ...
                "high_cloud_weighted": "log1p Huber delta=0.1; train weight 1+high_weight*clip(CPP_COT/30,0,1)",
                "input_domain": "observed real AGRI for both train and validation; no forecast AGRI",
    ...
    for arm in ARMS:
        ...
        for epoch in range(args.epochs):
            ...
            for ids in permutation.split(args.batch):
                ...
                loss = train_loss(prediction, y[ids], mask[ids], arm, args.high_weight)
    ...
    candidate = json.loads((output / "high_cloud_weighted/best_metrics.json").read_text())
    result = {"state": "COMPLETE_PAIRED_VALIDATION_PILOT", "test_used": False,
              "control": control, "high_cloud_weighted": candidate,
              ...
              "note": "Both arms train and validate on real observed AGRI; validation-only selection; no independent test or GHI claim."}
